    ...
    var KsYLabels = AbstractField.extend({
        ...
        ks_check_for_labels: function(){
            ...
            if(self.value){
                ...
                Object.keys(query_result.records[0]).forEach(function(key){
                    for(var i =0;i< query_result.records.length; i++){
                        if(query_result.records[i][key] === null){
                            continue;
                        }
                        if(typeof(query_result.records[i][key]) === "number") {
                            var ks_row = {}
